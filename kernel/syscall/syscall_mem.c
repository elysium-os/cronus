...
syscall_return_t syscall_mem_anon_allocate(size_t size) {
    ...
    if(size == 0 || size % PAGE_GRANULARITY != 0) {
        ret.error = SYSCALL_ERROR_INVALID_VALUE;
        return ret;
    }

    void *ptr = vm_map_anon(sched_thread_current()->proc->address_space, nullptr, size, (vm_protection_t) { .read = true, .write = true }, VM_CACHE_STANDARD, VM_FLAG_ZERO);
...
syscall_return_t syscall_mem_anon_free(void *pointer, size_t size) {
    syscall_return_t ret = {};
    if(size == 0 || size % PAGE_GRANULARITY != 0 || pointer == nullptr || ((uintptr_t) pointer) % PAGE_GRANULARITY != 0) {
        ret.error = SYSCALL_ERROR_INVALID_VALUE;
        return ret;
    }

    // CRITICAL: ensure this is safe for userspace to just do (currently throws a kern panic...)
    vm_unmap(sched_thread_current()->proc->address_space, pointer, size);
